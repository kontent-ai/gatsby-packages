...
const resolveUrls = (api) => {
  const { actions: { createTypes }, schema } = api;

  const urlSlugExtended =
      ...
      fields: {
        other_languages: {
          ...
          resolve: async (source, args, context, info) => {
            const pluginInfo = await context.nodeModel
              .runQuery({
                query: {
                  filter: {
                    name: {
            ...
            const otherLanguageCodenames = languageCodenames.filter(lang => lang !== source.preferred_language);

            // Including language fallback
            const otherLanguageNodes = await context.nodeModel
              .runQuery({
                query: {
                  filter: {
                    system: {
      ...
      fields: {
        content_items: {
          ...
          resolve: async (source, args, context, info) => {
            ...
            const linkedContentItems = await context.nodeModel
              .runQuery({
                query: {
                  filter: {
                    system: {
...
const linkUsedByContentItems = (api, parentTypeCodename, childTypeCodename, linkedElementCodename, backReferenceName) => {
  const { actions: { createTypes }, schema } = api;
      ...
      fields: {
        [backReferenceName]: {
          ...
          resolve: async (source, args, context, info) => {
            ...
            const linkedNodes = await context.nodeModel
              .runQuery({
                query: {
                  filter: {
                    elements: {
